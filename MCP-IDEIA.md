# Ideia de Servidor MCP — EQ07

**Domínio:** Gestão de corridas/eventos esportivos (com geocoding)  
**Data:** 2026-07-01
...
## Servidor proposto: `corridas-mcp`

### Tools sugeridas

- `listar_corridas(proximas, cidade)` — corridas futuras
- `geocodificar_local(endereco)` — lat/long do local
- `inscrever_atleta(corridaId, userId)` — inscrição
- `detalhes_organizador(id)` — dados do organizador

### Resources (somente leitura)

- lista de corridas como resource

### Exemplos de uso com um LLM

- "Ache corridas de 10k perto de João Pessoa no próximo mês e me inscreva."
...
@Service
public class CorridasTools {

    private final SeuService seuService;   // injete seus services/repositories

    public CorridasTools(SeuService seuService) { this.seuService = seuService; }

    @Tool(description = "corridas futuras")
    public Object listar_corridas(/* params */) {
